...
class TestContainersHelper:
    ...
    def create_weasyprint_service_container(self, parameters: TestContainerParameters) -> str:
        ...
        try:
            ...
            self.logger.info("Weasyprint service in bridge network is accessible through: " + base_url)
    ...
    def create_polarion_container(self, extension_name: PolarionApiExtensionName, parameters: TestContainerParameters, weasyprint_service_endpoint: str) -> tuple[str, str | None]:
        ...
        try:
            ...
            self.logger.info("Polarion is accessible through: " + base_url)
            self.polarion_container = container

            self.logger.info("Waiting for Polarion http connection ...")
            time.sleep(10)
    ...
    def tear_down(self) -> None:
        ...
        if self.polarion_container is not None and self.polarion_container.get_wrapped_container() is not None:
            self.logger.info("Stopping Polarion test container and cleaning temp directory ...")
            self.polarion_container.stop()
        if self.systest_extensions_root is not None and Path(self.systest_extensions_root).exists():
    ...
    def copy_dependency(systest_extensions_jars_path: str, group_id: str, artifact_id: str, version: str) -> None:
        ...
        command = [mvn_path, "dependency:copy", f"-Dartifact={group_id}:{artifact_id}:{version}", f"-DoutputDirectory={systest_extensions_jars_path}"]
