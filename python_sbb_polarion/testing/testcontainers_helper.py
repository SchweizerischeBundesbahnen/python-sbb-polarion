...
DEFAULT_ADMIN_UTILITY_VERSION = "5.0.3"
DEFAULT_TEST_DATA_VERSION = "5.1.0"
...
class TestContainersHelper:
    ...
    @staticmethod
    def get_parameters(args: argparse.Namespace) -> PolarionContainerParameters:
        polarion_image_name: str | None = TestContainersHelper.get_parameter("TC_POLARION_IMAGE_NAME", args.tc_polarion_image_name)
        weasyprint_service_image_name: str | None = TestContainersHelper.get_parameter("TC_WEASYPRINT_SERVICE_IMAGE_NAME", args.tc_weasyprint_service_image_name)
    ...
    def get_parameter(env_param_name: str, script_argument_value: str | None) -> str | None:
        param: str | None = os.environ.get(env_param_name)
        if not param:
    ...
    def parse_additional_bundles(bundles: str | None) -> list[ArtifactInfo] | None:
        if bundles is None:
            return None
        bundles_list: list[str] = bundles.split(",")
        artifacts_info_list: list[ArtifactInfo] = []
        for bundle in bundles_list:
            artifact_details: list[str] = bundle.split(":")
    ...
    def create_weasyprint_service_container(self, parameters: PolarionContainerParameters) -> str:
        ...
        try:
            ...
            container: DockerContainer = DockerContainer(image=parameters.weasyprint_service_image_name).with_bind_ports(port).with_name(container_name)
            container.start()
            if self.network:
    ...
    def create_polarion_container(self, extension_name: str, parameters: PolarionContainerParameters, weasyprint_service_endpoint: str | None) -> tuple[str, str]:
        ...
        try:
            ...
            token: str = self.setup_polarion_container(base_url)
        except Exception as ex:
            self.tear_down()
    ...
    def setup_polarion_container(base_url: str) -> str:
        polarion_connection: PolarionRestApiConnection = PolarionRestApiConnection(url=base_url, username=INITIAL_LOGIN, password=INITIAL_LOGIN)
        polarion_admin_utility_api: PolarionAdminUtilityApi = ExtensionApiFactory.get_extension_api_by_name(extension_name="admin-utility", polarion_connection=polarion_connection)
    ...
    def check_default_activation_response(activate_response: Response) -> None:
        content_type: str | None = activate_response.headers.get(Header.CONTENT_TYPE)
        if content_type is not None and MediaType.PLAIN in content_type and activate_response.content is not None and '"activated":true' in activate_response.content.decode():
    ...
    def copy_dependency(systest_extensions_jars_path: str, group_id: str, artifact_id: str, version: str | None) -> None:
        mvn_path: str = TestContainersHelper.get_maven_location()
        if not version:
            version = TestContainersHelper.get_latest_artifact_version(group_id, artifact_id)
        command: str = f"{mvn_path} dependency:copy -Dartifact={group_id}:{artifact_id}:{version} -DoutputDirectory={systest_extensions_jars_path}"
        # S602: shell=True is required for Maven command execution
        result: subprocess.CompletedProcess[bytes] = subprocess.run(command, check=False, shell=True)  # noqa: S602
    ...
    def get_latest_artifact_version(group_id: str, artifact_id: str) -> str:
        repo_location: str = TestContainersHelper.get_maven_repo_location()
        artifact_path: pathlib.Path = pathlib.Path(repo_location) / group_id.replace(".", "/") / artifact_id
        artifact_dir_content: list[str] = [item.name for item in artifact_path.iterdir()]
        regex: re.Pattern[str] = re.compile(r"^(?:\d+\.)?(?:\d+\.)?(?:\*|\d+)(?:-SNAPSHOT)?$")

        def is_version(dir_name: str) -> bool:
            return regex.match(dir_name) is not None

        versions: list[str] = list(filter(is_version, artifact_dir_content))
        versions.sort(reverse=True)
        return versions[0]

    @staticmethod
    def get_maven_repo_location() -> str:
        mvn_location: str = TestContainersHelper.get_maven_location()
        command: str = f"{mvn_location} -q help:evaluate -Dexpression=settings.localRepository -DforceStdout=true"
        # S602: shell=True is required for Maven command execution
        result: subprocess.CompletedProcess[str] = subprocess.run(command, check=False, shell=True, stdout=subprocess.PIPE, text=True)  # noqa: S602
        if result.returncode != 0:
...
@dataclass
class PolarionContainerParameters:
