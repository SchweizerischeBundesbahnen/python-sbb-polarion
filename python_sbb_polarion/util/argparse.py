...
def get_script_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--tc_weasyprint_api_key", type=str, help="API key a started bulk processing service sends to the WeasyPrint service it converts through (only over https)")
    parser.add_argument("--tc_bulk_processing_service_image_name", type=str, help="Bulk processing service docker image")
    parser.add_argument("--tc_bulk_processing_service_url", type=str, help="URL of an already-running bulk processing service; when set, no bulk processing container is started")
